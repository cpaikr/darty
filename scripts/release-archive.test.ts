...
const makeTempDir = () => {
  const directory = join(tmpdir(), `darty-release-archive-${crypto.randomUUID()}`);
...
describe("release archive creation", () => {
  test("creates a deterministic tar.gz containing the executable", () => {
    ...
    const archivePath = join(directory, "darty.tar.gz");
    ...
    expect(readTarString(tar, 0, 100)).toBe("darty");
  ...
  test("creates a zip containing the Windows executable", () => {
    const directory = makeTempDir();
    const archivePath = join(directory, "darty.zip");
    const executable = Buffer.from("MZ darty executable");
    ...
    expect(fileName).toBe("darty.exe");
  ...
  test("checksums files with sha256", () => {
    const directory = makeTempDir();
    const archivePath = join(directory, "darty.tar.gz");
    const executable = Buffer.from("darty");
