...
export function completeNpmPublication(
  ...
  if (versionLookup.status === 0) {
    ...
    const localPack = runNpm(["pack", "--ignore-scripts", "--dry-run", "--json"]);
    if (localPack.status !== 0) {
      fail(`Could not inspect the local npm package: ${formatFailure(localPack)}`);
    }
    const packResults = parseJson(localPack, "the local package");
    const localIntegrity = Array.isArray(packResults) ? packResults[0]?.integrity : null;
    if (typeof localIntegrity !== "string") {
      fail("The local npm package has no generated integrity.");
    }
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!option?.startsWith("--") || value === undefined) {
      fail("Usage: publish-npm-package.mjs --name @SCOPE/NAME --version X.Y.Z");
    }
...
async function main() {
  ...
  const result = completeNpmPublication(input, systemNpm);
