...
const temporaryRoot = mkdtempSync(join(tmpdir(), "darty-sdk-consumer-"));
...
try {
  ...
  const addonName = process.platform === "darwin" ? "libdarty_node.dylib" : "libdarty_node.so";
  const productionAddonArtifact = process.env.DARTY_PRODUCTION_ADDON ?? join(repoRoot, "target/release", addonName);
  ...
  const configuredFixtureTarget = process.env.DARTY_SDK_FIXTURE_TARGET;
  ...
  const fixtureAddonArtifact = join(fixtureTarget, "release", addonName);
  ...
  run(
    ...
    ["-p", join(repoRoot, "packages/node/tsconfig.json"), "--outDir", generatedFacade],
  );
  for (const generatedFile of ["index.js", "index.d.ts"]) {
    assert.equal(
      readFileSync(join(repoRoot, "packages/node", generatedFile), "utf8"),
      readFileSync(join(generatedFacade, generatedFile), "utf8"),
      `${generatedFile} must be fresh from the typed facade source`,
    );
  }
  const facadeSource = readFileSync(join(repoRoot, "packages/node/index.ts"), "utf8");
  assert.doesNotMatch(facadeSource, /DARTY_(?:NODE_TEST_)?FIXTURE/);
  assert.doesNotMatch(facadeSource, /process\.env/);

  const stagePackage = (name, addonArtifact) => {
    const rootPackage = join(temporaryRoot, `${name}-stage`);
    cpSync(join(repoRoot, "packages/node"), rootPackage, { recursive: true });
    copyFileSync(addonArtifact, join(rootPackage, "darty.node"));
    const tarballs = join(temporaryRoot, `${name}-tarballs`);
    mkdirSync(tarballs);
    const [rootPack] = JSON.parse(run("npm", ["pack", rootPackage, "--ignore-scripts", "--json", "--pack-destination", tarballs]).stdout);
    assert.deepEqual(rootPack.files.map(({ path }) => path).sort(), ["LICENSE.md", "darty.node", "index.d.ts", "index.js", "native.js", "package.json"]);
    return { rootPack, tarballs };
  };
  const productionPackages = process.env.DARTY_NODE_TARBALL
    ? { tarballs: dirname(resolve(process.env.DARTY_NODE_TARBALL)), rootPack: { filename: resolve(process.env.DARTY_NODE_TARBALL).split("/").at(-1) } }
    : stagePackage("production", productionAddonArtifact);
  const fixturePackages = stagePackage("fixture", fixtureAddonArtifact);
  const installConsumer = (name, packages) => {
    const consumer = join(temporaryRoot, name);
    mkdirSync(consumer);
    writeFileSync(join(consumer, "package.json"), JSON.stringify({ private: true, type: "module" }));
    run("npm", ["install", "--ignore-scripts", "--offline", "--no-audit", "--no-fund", join(packages.tarballs, packages.rootPack.filename)], { cwd: consumer });
    return consumer;
  };
...
`);
  ...
  assert.equal(existsSync(join(consumer, "node_modules/.bin/darty")), false, "SDK package must not install another CLI");
  ...
  console.log("Rust/Node packaged-consumer validation passed.");
} finally {
