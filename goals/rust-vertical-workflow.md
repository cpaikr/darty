...
- PR feedback tightened help parity to exact option sets plus normalized semantic copy, clarified the company-detail locator, and pinned the new Node action. CI exposed a pre-existing `.js`/ESM smoke-fixture mismatch at the declared Node 20 minimum; changing only that temporary artifact to `.mjs` restored all 338 tests under Node 20.18.1.
